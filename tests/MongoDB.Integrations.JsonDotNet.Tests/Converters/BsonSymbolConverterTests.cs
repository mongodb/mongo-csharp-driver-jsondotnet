/* Copyright 2015 MongoDB Inc.
*
* Licensed under the Apache License, Version 2.0 (the "License");
* you may not use this file except in compliance with the License.
* You may obtain a copy of the License at
*
* http://www.apache.org/licenses/LICENSE-2.0
*
* Unless required by applicable law or agreed to in writing, software
* distributed under the License is distributed on an "AS IS" BASIS,
* WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
* See the License for the specific language governing permissions and
* limitations under the License.
*/

using System;
using FluentAssertions;
using MongoDB.Bson;
using MongoDB.Integrations.JsonDotNet.Converters;
using NUnit.Framework;

namespace MongoDB.Integrations.JsonDotNet.Tests.Converters
{
    [TestFixture]
    public class BsonSymbolConverterTests : JsonConverterTestsBase
    {
        [Test]
        public void Instance_get_returns_cached_result()
        {
            var result1 = BsonSymbolConverter.Instance;
            var result2 = BsonSymbolConverter.Instance;

            result2.Should().BeSameAs(result1);
        }

        [Test]
        public void Instance_get_returns_expected_result()
        {
            var result = BsonSymbolConverter.Instance;

            result.Should().NotBeNull();
            result.Should().BeOfType<BsonSymbolConverter>();
        }

        [TestCase("{ x : null }", null)]
        [TestCase("{ x : \"abc\" }", "abc")]
        [TestCase("{ x : \"def\" }", "def")]
        [TestCase("{ x : { $symbol : \"abc\" } }", "abc")]
        [TestCase("{ x : { $symbol : \"def\" } }", "def")]
        public void ReadJson_should_return_expected_result_when_using_native_bson_reader(string json, string nullableName)
        {
            var subject = new BsonSymbolConverter();
            var expectedResult = nullableName == null ? null : BsonSymbolTable.Lookup(nullableName);

            var result = ReadJsonUsingNativeBsonReader<BsonSymbol>(subject, ToBson(json), mustBeNested: true);

            result.Should().BeSameAs(expectedResult);
        }

        [TestCase("null", null)]
        [TestCase("\"abc\"", "abc")]
        [TestCase("\"def\"", "def")]
        [TestCase("{ $symbol : \"abc\" }", "abc")]
        [TestCase("{ $symbol : \"def\" }", "def")]
        public void ReadJson_should_return_expected_result_when_using_native_json_reader(string json, string nullableName)
        {
            var subject = new BsonSymbolConverter();
            var expectedResult = nullableName == null ? null : BsonSymbolTable.Lookup(nullableName);

            var result = ReadJsonUsingNativeJsonReader<BsonSymbol>(subject, json);

            result.Should().BeSameAs(expectedResult);
        }

        [TestCase("{ x : null }", null)]
        [TestCase("{ x : \"abc\" }", "abc")]
        [TestCase("{ x : \"def\" }", "def")]
        [TestCase("{ x : { $symbol : \"abc\" } }", "abc")]
        [TestCase("{ x : { $symbol : \"def\" } }", "def")]
        [TestCase("{ x : { $$symbol : \"abc\" } }", "abc")]
        [TestCase("{ x : { $$symbol : \"def\" } }", "def")]
        public void ReadJson_should_return_expected_result_when_using_wrapped_bson_reader(string json, string nullableName)
        {
            var subject = new BsonSymbolConverter();
            var expectedResult = nullableName == null ? null : BsonSymbolTable.Lookup(nullableName);

            var result = ReadJsonUsingWrappedBsonReader<BsonSymbol>(subject, ToBson(json), mustBeNested: true);

            result.Should().BeSameAs(expectedResult);
        }

        [TestCase("null", null)]
        [TestCase("\"abc\"", "abc")]
        [TestCase("\"def\"", "def")]
        [TestCase("{ $symbol : \"abc\" }", "abc")]
        [TestCase("{ $symbol : \"def\" }", "def")]
        public void ReadJson_should_return_expected_result_when_using_wrapped_json_reader(string json, string nullableName)
        {
            var subject = new BsonSymbolConverter();
            var expectedResult = nullableName == null ? null : BsonSymbolTable.Lookup(nullableName);

            var result = ReadJsonUsingWrappedJsonReader<BsonSymbol>(subject, json);

            result.Should().BeSameAs(expectedResult);
        }

        [Test]
        public void ReadJson_should_throw_when_token_type_is_invalid()
        {
            var subject = new BsonSymbolConverter();
            var json = "undefined";

            Action action = () => { var _ = ReadJsonUsingNativeJsonReader<BsonSymbol>(subject, json); };

            action.ShouldThrow<Newtonsoft.Json.JsonReaderException>();
        }

        [TestCase(null, "{ x : null }")]
        [TestCase("abc", "{ x : { $$symbol : \"abc\" } }")] // note: Json.NET can't write a BsonSymbol
        [TestCase("def", "{ x : { $$symbol : \"def\" } }")] // note: Json.NET can't write a BsonSymbol
        public void WriteJson_should_have_expected_result_when_using_native_bson_writer(string nullableName, string expectedResult)
        {
            var subject = new BsonSymbolConverter();
            var value = nullableName == null ? null : BsonSymbolTable.Lookup(nullableName);

            var result = WriteJsonUsingNativeBsonWriter(subject, value, mustBeNested: true);

            result.Should().Equal(ToBson(expectedResult));
        }

        [TestCase(null, "null")]
        [TestCase("abc", "{\"$symbol\":\"abc\"}")]
        [TestCase("def", "{\"$symbol\":\"def\"}")]
        public void WriteJson_should_have_expected_result_when_using_native_json_writer(string nullableName, string expectedResult)
        {
            var subject = new BsonSymbolConverter();
            var value = nullableName == null ? null : BsonSymbolTable.Lookup(nullableName);

            var result = WriteJsonUsingNativeJsonWriter(subject, value);

            result.Should().Be(expectedResult);
        }

        [TestCase(null, "{ x : null }")]
        [TestCase("abc", "{ x : { $symbol : \"abc\" } }")]
        [TestCase("def", "{ x : { $symbol : \"def\" } }")]
        public void WriteJson_should_have_expected_result_when_using_wrapped_bson_writer(string nullableName, string expectedResult)
        {
            var subject = new BsonSymbolConverter();
            var value = nullableName == null ? null : BsonSymbolTable.Lookup(nullableName);

            var result = WriteJsonUsingWrappedBsonWriter(subject, value, mustBeNested: true);

            result.Should().Equal(ToBson(expectedResult));
        }

        [TestCase(null, "null")]
        [TestCase("abc", "{ \"$symbol\" : \"abc\" }")]
        [TestCase("def", "{ \"$symbol\" : \"def\" }")]
        public void WriteJson_should_have_expected_result_when_using_wrapped_json_writer(string nullableName, string expectedResult)
        {
            var subject = new BsonSymbolConverter();
            var value = nullableName == null ? null : BsonSymbolTable.Lookup(nullableName);

            var result = WriteJsonUsingWrappedJsonWriter(subject, value);

            result.Should().Be(expectedResult);
        }
    }
}
